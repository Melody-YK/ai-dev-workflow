...
If the assistant is showing clarification questions to the user, phase 01 is not complete yet. Do not write `COMPLETE`, `PASSED`, or `DONE` for 01 in `STATUS.md` or `01_REQUIREMENTS.md`; use `NEEDS_HUMAN_INPUT` until the user answers and the answers are persisted to `clarification.md`, `open-questions.md`, `STATUS.md`, and traceability where relevant.
